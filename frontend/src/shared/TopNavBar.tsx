    ...
    >
      <Toolbar sx={{ position: 'relative', zIndex: 1 }}>
        ...
        <Box display="flex" alignItems="center" gap={2}>
          <Box display="flex" alignItems="center" gap={2}>
            <Button onClick={() => navigate('/home')} sx={navBtnSx(location.pathname === '/home')}>
            ...
            <Button onClick={() => navigate('/news')} sx={navBtnSx(location.pathname === '/news')}>
              <Box sx={{ position: 'relative', display: 'inline-flex', alignItems: 'center' }}>
                优选
            ...
            <Button onClick={() => navigate('/pricing')} sx={navBtnSx(location.pathname === '/pricing')}>
              <Box sx={{ position: 'relative', display: 'inline-flex', alignItems: 'center' }}>
                会员套餐
          ...
          <Box display="flex" alignItems="center" gap={1} sx={{ position: 'relative' }}>
            <Box sx={{ p: 0, cursor: 'pointer' }} onMouseEnter={handleMenuOpen}>
              <Avatar sx={{ bgcolor: '#2563eb', width: 32, height: 32, fontSize: '0.9rem' }}>
                {loading ? '...' : user?.username ? String(user.username).charAt(0).toUpperCase() : 'U'}
              </Avatar>
            </Box>
            <Menu
              anchorEl={anchorEl}
            ...
            >
              ...
              <MenuItem
                ...
                sx={{ py: 0.8, px: 2, fontSize: '0.75rem', minHeight: 'auto' }}
              >
                <Typography variant="body2" sx={{ color: '#334155', fontWeight: 500, fontSize: '0.75rem' }}>
                  个人中心（积分购买）
                </Typography>
              </MenuItem>
              <MenuItem onClick={handleLogout} sx={{ py: 0.8, px: 2, fontSize: '0.75rem', minHeight: 'auto' }}>
                <Typography variant="body2" sx={{ color: '#ef4444', fontWeight: 500, fontSize: '0.75rem' }}>
                  退出登录
                </Typography>
              </MenuItem>
            </Menu>
